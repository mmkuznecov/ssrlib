...
class Pipeline:
    ...
    def _setup_storage(self, storage_dir: str, description: str = "") -> TensorStorage:
        ...
        """
        if os.path.exists(storage_dir) and os.path.exists(
            os.path.join(storage_dir, "metadata", "metadata.json")
        ):
    ...
    def execute(
        ...
        if use_storage and storage and not force_recompute:
            cached_embeddings = self._load_embeddings_from_storage(
                storage, storage_keys_needed
            )
        ...
        for dataset in self.datasets:
            ...
            for embedder in self.embedders:
                ...
                # Store metadata
                results.metadata[f"{dataset_key}_{embedder.name}_shape"] = (
                    embeddings.shape
                )
                results.metadata[f"{dataset_key}_{embedder.name}_dtype"] = str(
                    embeddings.dtype
                )
            ...
            for dataset in self.datasets:
                ...
                for embedder in self.embedders:
                    ...
                    for processor in self.processors:
                        print(
                            f"Processing {dataset_key}-{embedder.name} with {processor.name}"
                        )

                        processed = processor.process(embeddings)
                        results.processed[
                            (dataset_key, embedder.name, processor.name)
                        ] = processed
