        ...
        try:
            ...
            for name, obj in inspect.getmembers(module, inspect.isclass):
                ...
                if (
                    ...
                    registry.register(
                        name, obj, description, category, output_type, properties
                    )

                    logger.debug(
                        f"Registered processor: {name} ({category}/{output_type})"
                    )
...
if logger.isEnabledFor(logging.INFO):
    logger.info(
        f"Processor discovery complete: {len(_processor_registry.list_processors())} processors found"
    )
