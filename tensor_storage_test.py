...
def test_large_scale_storage():
    ...
    def generate_test_metadata() -> Iterator[Dict[str, Any]]:
        ...
        for i in range(num_vectors):
            meta = {
                ...
                'dataset_split': ['train', 'val', 'test'][i % 3],  # Cycle through splits
                'timestamp': f"2024-{(i // 50000) + 1:02d}-01",    # Monthly timestamps (20 months)
                'vector_norm': float((i * 7) % 1000) / 1000.0,     # Synthetic norm values [0, 1)
                'quality_score': 0.5 + 0.5 * np.sin(i * 0.001),   # Sine wave quality scores
                'processing_time': 0.001 + (i % 100) * 0.00001,    # Synthetic processing times
            }
            yield meta
    ...
    def verify_vector_sample(storage: TensorStorage, sample_size: int = 20) -> None:
        ...
        for idx in sample_indices:
            ...
            assert retrieved_vector.shape == (vector_size,), \
                f"Shape mismatch at index {idx}: expected {(vector_size,)}, got {retrieved_vector.shape}"
            
            assert np.allclose(retrieved_vector, expected_vector, rtol=1e-6, atol=1e-6), \
    ...
    def test_metadata_queries(storage: TensorStorage) -> None:
        ...
        assert len(batch_0_vectors) == 1000, f"Expected 1000 vectors in batch 0, got {len(batch_0_vectors)}"
        
        # Test model version filtering
        v0_indices = storage.filter_tensors(model_version='v0')
        assert len(v0_indices) == 250000, f"Expected 250k vectors for model v0, got {len(v0_indices)}"
        
        # Test dataset split filtering
        train_indices = storage.filter_tensors(dataset_split='train')
        expected_train_count = num_vectors // 3 + (1 if num_vectors % 3 > 0 else 0)  # Ceiling division
        assert len(train_indices) == expected_train_count, \
            f"Expected ~{expected_train_count} train vectors, got {len(train_indices)}"
        
        # Test multi-parameter filtering
        train_v0_indices = storage.filter_tensors(model_version='v0', dataset_split='train')
    ...
    def run_performance_test(chunk_size: int, chunk_label: str) -> Dict[str, float]:
        ...
        with tempfile.TemporaryDirectory() as temp_dir:
            ...
            try:
                ...
                assert len(storage) == num_vectors, \
    ...
    if len(all_metrics) > 1:
        ...
        print(f"{'Configuration':<15} {'Creation (s)':<12} {'Throughput (GB/s)':<17} {'Storage (MB)':<13} {'Chunks':<8}")
        print("-" * 75)
        
        for config, metrics in all_metrics.items():
            print(f"{config:<15} {metrics['creation_time']:<12.2f} "
                  f"{metrics['throughput_gb_per_s']:<17.3f} "
                  f"{metrics['storage_size_mb']:<13.1f} "
                  f"{metrics['num_chunks']:<8}")
        
        # Find best performing configuration
        best_config = max(all_metrics.items(), key=lambda x: x[1]['throughput_gb_per_s'])
    ...
    with tempfile.TemporaryDirectory() as temp_dir:
        ...
        try:
            ...
            try:
                ...
                assert len(rebuilt_storage) == len(original_storage), "Length should match"
                assert rebuilt_storage.chunk_size == 32 * 1024 * 1024, "Chunk size should be updated"
                ...
                for idx in sample_indices:
                    ...
                    assert np.allclose(original_tensor, rebuilt_tensor), f"Tensor mismatch at index {idx}"
                ...
                print(f"  Original: {original_info['total_size_mb']:.1f}MB, {original_storage.metadata['total_chunks']} chunks")
                print(f"  Rebuilt:  {rebuilt_info['total_size_mb']:.1f}MB, {rebuilt_storage.metadata['total_chunks']} chunks")
